...
const loadProductoDetalle = async () => {
    ...
    try {
        ...
        if (yaReseno.value) {
            const miResena = producto.value.resenas.find(r => r.nombreUsuario === authStore.user?.nombre);
            if (miResena) {
...
const submitResena = async () => {
    ...
    try {
        ...
        resenaError.value = 'Error al enviar la reseña. ' + (err.response?.data || err.message);
    } finally {
...
const formatDate = (dateString) => {
    return new Date(dateString).toLocaleDateString('es-GT', { year: 'numeric', month: 'short', day: 'numeric' });
};
...
<template>
    <div class="container py-5">
        <router-link :to="{ name: 'vendedor-dashboard' }" class="btn btn-outline-secondary btn-sm mb-4">
                ...
                <div class="col-md-7">
                    ...
                    <div class="d-flex align-items-center mb-3">
                        <div class="fs-5 me-3 text-warning">
                            <i v-for="(isFilled, index) in getStars(producto.promedioCalificaciones)" 
                                :key="'star-avg-' + index" 
                                :class="isFilled ? 'bi bi-star-fill' : 'bi bi-star'">
                            </i>
                        </div>
                        <span class="text-muted">({{ parseFloat(producto.promedioCalificaciones).toFixed(2) }} de 5, {{ producto.cantidadResenas }} reseñas)</span>
                    </div>
                    ...
                    <p class="mb-1" :class="producto.stock > 0 ? 'text-success' : 'text-danger'">
                        <strong>Disponibilidad:</strong> {{ producto.stock > 0 ? `En Stock (${producto.stock})` : 'Agotado' }}
                    </p>
                    <p class="mb-4"><strong>Condición:</strong> <span class="badge" :class="producto.esNuevo ? 'bg-info' : 'bg-warning'">{{ producto.esNuevo ? 'Nuevo' : 'Usado' }}</span></p>

                    <button 
                        class="btn btn-success btn-lg w-100 mb-5" 
                        :disabled="producto.stock === 0"
                    >
                        <i class="bi bi-cart-plus me-2"></i> {{ producto.stock > 0 ? 'Agregar al Carrito' : 'Sin Stock' }}
                    </button>
                </div>
            </div>
                ...
                <div class="col-12">
                    <ul class="nav nav-tabs" id="productTabs" role="tablist">
                        <li class="nav-item" role="presentation">
                            <button class="nav-link active" id="desc-tab" data-bs-toggle="tab" data-bs-target="#desc-pane" type="button" role="tab" aria-controls="desc-pane" aria-selected="true">Descripción</button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button class="nav-link" id="resenas-tab" data-bs-toggle="tab" data-bs-target="#resenas-pane" type="button" role="tab" aria-controls="resenas-pane" aria-selected="false">Reseñas ({{ producto.cantidadResenas }})</button>
                        </li>
                    </ul>
                    ...
                    <div class="tab-content pt-3" id="productTabsContent">
                        ...
                        <div class="tab-pane fade" id="resenas-pane" role="tabpanel" aria-labelledby="resenas-tab" tabindex="0">
                            
                            <div v-if="authStore.isAuthenticated" class="card p-4 mb-4 bg-light">
                                <h5 class="mb-3">{{ yaReseno ? 'Actualizar tu Reseña' : 'Dejar una Reseña' }}</h5>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <div class="star-rating">
                                            <i v-for="n in 5" 
                                               ...
                                               :class="n <= resenaForm.calificacion ? 'bi bi-star-fill text-warning' : 'bi bi-star text-muted'"
                                               class="fs-3 cursor-pointer me-1">
                                            </i>
                                            <span class="ms-2 text-muted">{{ resenaForm.calificacion }} de 5</span>
                                        </div>
                                    </div>

                                    <div class="mb-3">
                                        <label class="form-label">Comentario (Opcional)</label>
                                        <textarea class="form-control" v-model="resenaForm.comentario" rows="3"></textarea>
                                    </div>
                                    
                                    <div v-if="resenaError" class="alert alert-danger">{{ resenaError }}</div>
                                    <div v-if="resenaSuccess" class="alert alert-success">{{ resenaSuccess }}</div>

                                    <button type="submit" class="btn btn-primary" :disabled="isSendingResena || resenaForm.calificacion === 0">
                            ...
                            <div v-if="producto.resenas && producto.resenas.length">
                                <div v-for="resena in producto.resenas" :key="resena.nombreUsuario" class="border-bottom py-3">
                                    <p class="mb-1">
                                        <strong class="me-2">{{ resena.nombreUsuario }}</strong>
                                        <span class="text-warning">
                                            <i v-for="(isFilled, index) in getStars(resena.calificacion)" 
                                                :key="'star-res-' + index" 
                                                :class="isFilled ? 'bi bi-star-fill' : 'bi bi-star'">
                                            </i>
                                        </span>
                                    ...
                                    <p class="mb-0 text-secondary">{{ resena.comentario || 'Sin comentario.' }}</p>
                                </div>
                            </div>
                            <p v-else class="text-muted mt-3">Sé el primero en reseñar este producto.</p>
                        </div>
                    </div>
                </div>
            </div>
...
.star-rating i.bi-star-fill, .star-rating i.bi-star {
