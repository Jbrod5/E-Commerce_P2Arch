<template>
  <div class="container py-4">
            ...
            <tbody>
              <tr v-for="vendedor in vendedores" :key="vendedor.id">
                ...
                <td class="text-center">
                  <span class="badge" :class="vendedor.activo ? 'bg-success' : 'bg-danger'">
    ...
    <!-- Modal Sanción -->
    <div v-if="mostrarModalSancion" class="modal show d-block" style="background-color: rgba(0,0,0,0.5)">
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <p class="mb-3">
              Vendedor: <strong>{{ usuarioSeleccionado.nombre }}</strong> (ID: {{ usuarioSeleccionado.id }})
            </p>
          ...
          <div class="modal-footer">
            ...
            >
              <span v-if="cargandoPeticion" class="spinner-border spinner-border-sm me-2"></span>
              Confirmar Sanción
            </button>
          </div>
        </div>
    ...
    <!-- Modal Historial -->
    <div v-if="mostrarModalHistorial" class="modal show d-block" style="background-color: rgba(0,0,0,0.5)">
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">Historial de Sanciones</h5>
            <button type="button" class="btn-close" @click="cerrarModalHistorial"></button>
          </div>
          <div class="modal-body">
            ...
            <div v-else-if="historialSanciones.length > 0">
              <div v-for="sancion in historialSanciones" :key="sancion.id" class="card mb-2">
                <div class="card-body">
                  <div class="d-flex justify-content-between align-items-start">
                    <div>
                      <span class="badge me-2" :class="sancion.activa ? 'bg-danger' : 'bg-success'">
                        {{ sancion.activa ? 'Activa' : 'Finalizada' }}
                      </span>
                      <small class="text-muted">#{{ sancion.id }}</small>
                    </div>
                    <small class="text-muted">{{ formatoFecha(sancion.fechaSuspension) }}</small>
                  </div>
